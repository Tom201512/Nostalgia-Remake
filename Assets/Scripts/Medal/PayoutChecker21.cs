...
public class PayoutChecker21
{
    ...
    // コンストラクタ
    public PayoutChecker21(StreamReader normalPayout, StreamReader bigPayout, StreamReader jacPayout, 
    ...
    // ライン判定
    public void CheckPayoutLines(int betAmount, List<List<ReelData21.ReelSymbols>> lastSymbols)
    {
        // 最終的な払い出し結果
        int finalPayouts = 0;
        int bonusID = 0;
        ...
        {
            // 指定ラインの結果を保管
            List <ReelData21.ReelSymbols> lineResult = new List<ReelData21.ReelSymbols>();
    ...
    // 図柄の判定(配列を返す)
    private int CheckPayoutLines(List<ReelData21.ReelSymbols> lineResult, List<PayoutResultData> payoutResult)
    {
        // 全て同じ図柄が揃っていたらHITを返す
        // ANY(10番)は無視
