...
namespace ReelSpinGame_UI.Graph
{
    ...
    public class GraphComponent : MonoBehaviour
    {
        ...
        const float GraphMaxPosX = 1200.0f;                     // グラフX座標最大値
        const float GraphMaxPosY = 300.0f;                     // グラフY座標最大値
        const float GraphMiddlePosY = GraphMaxPosY / 2;         // Y座標中央値
        const float GraphXPosOffset = GraphMaxPosX / 1000;      // X座標分割値
        const float GraphYPosOffset = GraphMaxPosY / 1000;   // Y座標分割値
        ...
        public void StartDrawGraph(PlayerDatabase playerData)
        {
            ...
            if(playerData.PlayerMedalData.MedalSlumpGraph.Count > 0)
            {
                // 差枚数ごとに座標を求めて描画する
                while (true)
                {
