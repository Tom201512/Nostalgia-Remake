...
public class ReelObject21 : MonoBehaviour
{
    // リールオブジェクト
    ...
    // リール内の図柄
    private SymbolChange21[] symbolsObj;
    ...
    // リール情報
    public ReelData21 ReelData { get; private set; }
    ...
    private void Awake()
    {
        ...
        IsStopping = false;
        HasStopped = true;
        symbolsObj = GetComponentsInChildren<SymbolChange21>();

        Debug.Log(symbolsObj.Length);
    ...
    // func
    // リールデータを渡す
    public void SetReelData(ReelData21 reelData) => ReelData = reelData;
    // 指定位置からリール位置を渡す
    public int GetReelPos(int posID) => ReelData.GetReelPos((sbyte)posID);
    // 指定位置からリール図柄を渡す
    public ReelData21.ReelSymbols GetReelSymbol(int posID) => ReelData.GetReelSymbol((sbyte)posID);
    // 停止位置を返す
    public int GetStoppedPos() => ReelData.GetReelPos((int)ReelData21.ReelPosID.Center);
    ...
    private void UpdateSymbolsObjects()
    {
        ...
        {
            symbolsObj[i].ChangeSymbol(ReelData.GetReelSymbol21(i));
        }
    }
